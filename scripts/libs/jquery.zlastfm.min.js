		...
		if (feeds) {
			...
			for (var i=0; i<count; i++) {
				...
				if (options.image) {
					...
					if (imgurl != '') html += '<a class="artWrap" rel="external" href="'+ url +'" style="background-image: url('+ imgurl +')" title="Listen to '+ item.name +' on Last.FM"><img src="'+ imgurl +'" alt="'+ item.name +'" /></a>'
				}

				// Add name
				if (options.name) html += '<div class="itemName"><a href="'+ url +'" title="Listen to '+ item.name +' on Last.FM"><em>'+ item.name +'</em></a></div>'
